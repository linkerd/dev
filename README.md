...
### linkerd/dev/actions/setup-tools
...
- RUST_TAG is updated to the latest. Make sure you update this in concert with
  the other repos consuming the dev container, otherwise revert the version
  bump.
- HELM_VERSION is bumped to v4, but that needs to be amended manually to
  the latest on the v3 timeline

The following dependencies aren't yet handled by `just update-versions`, so
they need to be bumped manually:
...
If required, update `minimum-k8s` in `justfile`

### 4. Update Go Version

If required, update the default go version in `actions/setup-go/action.yml`.
...
`actions/setup-tools/action.yml` and push your change to the branch. This will
cause tests to fail given it cannot pull the new dev version we haven't pushed
yet. So after the PR is reviewed and the new release is ready, build and push
all the images directly from your branch:
